...
var PieChart = /** @class */ (function (_super) {
    ...
    PieChart.prototype.render = function () {
        ...
        var slices = chart.curves.map(function (c, i) {
            ...
            return (<G onPress={()=> {
                if(c.item.onPress!= null && c.item.onPress != undefined){
                         c.item.onPress()
                     }}} key={Math.random()}>
          <Path d={c.sector.path.print()} fill={c.item.color}/>
          {hasLegend ? (<Rect 
                 width="16px" height="16px" fill={c.item.color} rx={8} ry={8} x={_this.props.width / 2.5 - 24} y={-(_this.props.height / 2.5) +
                ((_this.props.height * 0.8) / _this.props.data.length) * i +
                12}/>) : null}
          {hasLegend ? (
              
          <Text  fill={c.item.legendFontColor} fontSize={c.item.legendFontSize} fontFamily={c.item.legendFontFamily} x={_this.props.width / 2.5} y={-(_this.props.height / 2.5) +
                ((_this.props.height * 0.8) / _this.props.data.length) * i +
                12 * 2}>
              {formatNumber(value) + " " + c.item.name}
            </Text>
